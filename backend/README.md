...
- Corre en el puerto **8000** (el core corre en 8001).
...
# 4) levantar el servidor (puerto 8000)
...
http POST :8000/auth/login username=cli000007 password=demo1234
```

### Consultas
```bash
# Cuentas de ahorro
curl http://127.0.0.1:8000/cuentas/ahorro -H "Authorization: Bearer $TOKEN"
http :8000/cuentas/ahorro "Authorization: Bearer $TOKEN"

# Movimientos de una cuenta de ahorro
curl "http://127.0.0.1:8000/cuentas/ahorro/AHO0000003/movimientos?limit=50" \
...
curl http://127.0.0.1:8000/cuentas/ahorro/PFI0000505/detalle \
  -H "Authorization: Bearer $TOKEN"

# Créditos del cliente (periodo 202512)
curl http://127.0.0.1:8000/cuentas/credito -H "Authorization: Bearer $TOKEN"

# Cronograma de cuotas de un crédito
curl http://127.0.0.1:8000/cuentas/credito/CRED0000006/cuotas \
  -H "Authorization: Bearer $TOKEN"
```
...
curl -X POST http://127.0.0.1:8000/operaciones/pago-cuota \
...
curl -X POST http://127.0.0.1:8000/operaciones/transferencia \
  -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
  -d '{"cuenta_origen":"AHO0000003","cuenta_destino":"APR0000703","monto":25.50}'

# Catálogo de servicios disponibles para pagar
curl http://127.0.0.1:8000/operaciones/servicios -H "Authorization: Bearer $TOKEN"

# Pago de servicio DEBITANDO una cuenta de ahorro (valida saldo)
curl -X POST http://127.0.0.1:8000/operaciones/pago-servicio \
  -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
...
curl -X POST http://127.0.0.1:8000/creditos/solicitar \
  -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
