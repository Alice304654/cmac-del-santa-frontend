...
def login(conn: Connection, username: str, password: str) -> dict:
    ...
    if not verificar_password(password, usuario["password_hash"]):
        intentos = repo_auth.registrar_login_fallido(
            conn,
            usuario["pkusuario"]
        )
    ...
    # Login exitoso
    repo_auth.registrar_login_exitoso(
        conn,
        usuario["pkusuario"]
    )
...
def registrar(conn: Connection, username: str, password: str) -> dict:
    try:
        usuario_existente = repo_auth.buscar_usuario_por_username(
            conn,
            username
        )
        ...
        # 🔐 encriptar password correctamente
