...
```bash
# Cuentas de ahorro
curl http://localhost:8000/cuentas/ahorro -H "Authorization: Bearer $TOKEN"
http :8000/cuentas/ahorro "Authorization: Bearer $TOKEN"

# Movimientos de una cuenta de ahorro
curl "http://localhost:8000/cuentas/ahorro/AHO0000003/movimientos?limit=50" \
...
curl http://localhost:8000/cuentas/ahorro/PFI0000505/detalle \
  -H "Authorization: Bearer $TOKEN"

# Créditos del cliente (periodo 202512)
curl http://localhost:8000/cuentas/credito -H "Authorization: Bearer $TOKEN"

# Cronograma de cuotas de un crédito
curl http://localhost:8000/cuentas/credito/CRED0000006/cuotas \
  -H "Authorization: Bearer $TOKEN"
```
...
curl -X POST http://localhost:8000/operaciones/pago-cuota \
...
curl -X POST http://localhost:8000/operaciones/transferencia \
  -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
  -d '{"cuenta_origen":"AHO0000003","cuenta_destino":"APR0000703","monto":25.50}'

# Catálogo de servicios disponibles para pagar
curl http://localhost:8000/operaciones/servicios -H "Authorization: Bearer $TOKEN"

# Pago de servicio DEBITANDO una cuenta de ahorro (valida saldo)
curl -X POST http://localhost:8000/operaciones/pago-servicio \
  -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
...
curl -X POST http://localhost:8000/creditos/solicitar \
  -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
